...
addon = xbmcaddon.Addon('plugin.video.bravo tv total')
...
Listas_Streem = os.path.join(home, 'Listas_Streem')
...
if os.path.exists(Listas_Streem)==True:
    SOURCES = open(Listas_Streem).read()
else: SOURCES = []


def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.bravo tv total-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(bravo tv total,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(bravo tv total,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        ...
        if os.path.exists(Listas_Streem)==True:
            sources = json.loads(open(Listas_Streem,"r").read())
...
def addSource(url=None):
        ...
        if os.path.exists(Listas_Streem)==False:
            source_list = []
            source_list.append(source_media)
            b = open(Listas_Streem,"w")
        ...
        xbmc.executebuiltin("XBMC.Notification(bravo tv total,New source added.,5000,"+icon+")")
        if not url is None:
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('bravo tv total','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('bravo tv total', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from PvPNetworkTutoriales Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('Add to PvPNetworkTutoriales Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from bravo tv total Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to bravo tv total Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(bravo tv total,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
